	...
	if(drawgrid==1)
	{
		...
		glBegin(GL_LINES);{
			for(i=-GRIDSIZE;i<=GRIDSIZE;i++){
				...
				glVertex3f(i*10, -GRIDSIZE*10, 0);
				glVertex3f(i*10,  GRIDSIZE*10, 0);

				//lines parallel to X-axis
				glVertex3f(-GRIDSIZE*10, i*10, 0);
				glVertex3f( GRIDSIZE*10, i*10, 0);
			}
		}glEnd();
	}
...
void drawBody()
{
	...
	glBegin(GL_QUADS);
		for(int i=0;i<4;i++)
		glVertex3d(bodyLengthHalf*dx[i],bodyWidthHalf*dy[i],0);
	glEnd();
	glColor3f(1.0,0,0);
	glBegin(GL_LINES);
		glVertex3d(0,0,0);
	...
	glBegin(GL_LINES);
		glVertex3d(0,0,10);
		glVertex3d(
			bodyLengthHalf*2*cos(wheelUpRotation*pi/180) ,
			bodyLengthHalf*2*sin(wheelUpRotation*pi/180),
			10); 
	glEnd();
...
{
	glPushMatrix();
	glTranslated(wheelPosition.x,wheelPosition.y,wheelPosition.z+2*wheelRadius);
	drawBody();
	glPopMatrix();
	for(int i=0;i<4;i++)
	{
		glPushMatrix();
		glTranslated(bodyLengthHalf*dx[i] , bodyWidthHalf*dy[i],0);
